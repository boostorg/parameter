...
// empty_arg_list.
//
// Each specialization of arg_list is derived from its successor in the list
// type.  This feature is used along with using declarations to build member
// function overload sets that can match against keywords.
//

// MPL sequence support
struct arg_list_tag;

// Terminates arg_list<> and represents an empty list.  Since this is just the
// terminating case, you might want to look at arg_list first to get a feel
// for what's really happening here.
struct empty_arg_list
{
    ...
    // Constructor taking BOOST_PARAMETER_MAX_ARITY empty_arg_list arguments;
    // this makes initialization.
    inline empty_arg_list(
    ...
    inline T& get(default_<K, T> const& x) const
    {
        return x.value;
    }
    ...
    get(lazy_default<K, F> const& x) const
    {
        return x.compute_default();
    }
#endif // Borland workarounds needed.

    // If this function is called, it means there is no argument in the list
    // that matches the supplied keyword.  Just return the default value.
    template <class K, class Default>
    inline Default& operator[](default_<K, Default> x) const
    {
        return x.value;
    }

    // If this function is called, it means there is no argument in the list
    // that matches the supplied keyword.  Just evaluate and return the
    // default value.
    template <class K, class F>
    inline typename result_of0<F>::type
...
// TaggedArg is an instance of tagged_argument<> or tagged_argument_rref<>.
...
struct arg_list : Next
{
...
#if defined BOOST_PARAMETER_HAS_PERFECT_FORWARDING
    // Use tag dispatching to determine whether to forward all arguments to
    // the Next constructor, or store the first argument and forward the rest.
    // -- Cromwell D. Enage
    template <class A0>
    inline arg_list(value_type_is_not_void, A0&& a0)
      : Next(
            typename mpl::if_<
                is_same<typename Next::tagged_arg::value_type, void_>
              , value_type_is_void
    ...
    template <class ...Args>
    inline arg_list(value_type_is_void, Args&&... args)
      : Next(
            typename mpl::if_<
                is_same<typename Next::tagged_arg::value_type, void_>
              , value_type_is_void
      ...
      , arg(aux::void_reference())
    {
    }

    template <class A0, class A1, class ...Args>
    inline arg_list(value_type_is_not_void, A0&& a0, A1&& a1, Args&&... args)
      : Next(
            typename mpl::if_<
                is_same<typename Next::tagged_arg::value_type, void_>
              , value_type_is_void
    ...
    struct binding
    {
        ...
        struct apply
        {
            typedef typename mpl::eval_if<
              ...
              , mpl::apply_wrap3<
                    typename Next::binding, KW, Default, Reference
                >
            >::type type;
        };
    };
    ...
    BOOST_MPL_ASSERT_MSG(
        sizeof(
            Next::has_key(static_cast<key_type*>(BOOST_TTI_DETAIL_NULLPTR))
        ) == sizeof(no_tag)
      , duplicate_keyword
      , (key_type)
    );
    ...
    template <class D>
    inline reference get_default(D const&, mpl::false_) const
    {
        return arg.get_value();
    }

    template <class D>
    inline reference get_default(D const& d, mpl::true_) const
    {
        return (
            arg.get_value()
          ? arg.get_value().get()
          : arg.get_value().construct(d.value)
        );
    }
    ...
    // A metafunction class that, given a keyword, returns the base sublist
    // whose get() function can produce the value for that key.
    struct key_owner
    {
        template<class KW>
        struct apply
    ...
    // get() function.
    template <class KW>
    inline typename mpl::apply_wrap3<binding, KW, void_, mpl::true_>::type
    ...
    // indicating no matching argument was passed, the default is returned,
    // or if no default_ or lazy_default was passed, compilation fails.
    inline reference get(keyword<key_type> const&) const
    {
    ...
    inline reference get(default_<key_type, Default> const& d) const
    {
        return get_default(d, holds_maybe());
    }

    template <class Default>
    inline reference get(lazy_default<key_type, Default> const&) const
    {
        return arg.get_value();
    }
#else // !BOOST_WORKAROUND(__BORLANDC__, BOOST_TESTED_AT(0x564))
    inline reference operator[](keyword<key_type> const&) const
    {
        BOOST_MPL_ASSERT_NOT((holds_maybe));
        return arg.get_value();
    }
    ...
    template <class Default>
    inline reference operator[](default_<key_type, Default> const& d) const
    {
        return get_default(d, holds_maybe());
    }

    template <class Default>
    inline reference operator[](lazy_default<key_type, Default> const&) const
    {
        BOOST_MPL_ASSERT_NOT((holds_maybe));
        return arg.get_value();
    }

    // Builds an overload set including operator[]s defined in base classes.
    using Next::operator[];
    ...
    // For parameter_requirements matching this node's key_type, return a bool
    // constant wrapper indicating whether the requirements are satisfied by
    // TaggedArg.  Used only for compile-time computation and never really
    // called, so a declaration is enough.
    ...
    inline arg_list<tagged_argument<KW, T2>, self>
    operator,(tagged_argument<KW, T2> const& x) const
    {
...
#if defined BOOST_PARAMETER_HAS_PERFECT_FORWARDING
    template <class KW, class T2>
    inline arg_list<tagged_argument_rref<KW, T2>, self>
    operator,(tagged_argument_rref<KW, T2> const& x) const
    {
        return arg_list<tagged_argument_rref<KW,T2>, self>(x, *this);
    }
#endif
